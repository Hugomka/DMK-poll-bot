    ...
    async def _handle_download(self, interaction: discord.Interaction) -> None:
        ...
        try:
            ...
            if not archive_exists_scoped(gid, cid):
                await interaction.followup.send(
                    t(cid, "ARCHIVE.no_archive"), ephemeral=True
                )
                return
            ...
            if ArchiveView is None:
                ...
                if not data:
                    await interaction.followup.send(
                        t(cid, "ARCHIVE.read_failed"), ephemeral=True
                    )
                    return

                await interaction.followup.send(
                    content=t(cid, "ARCHIVE.description"),
            ...
            # Weekend bericht
            weekend_content = t(cid, "ARCHIVE.archive_message_weekend")
            ...
            if weekday_csv_check:
                ...
                if weekday_csv:  # Type guard voor None check
                    weekday_content = t(cid, "ARCHIVE.archive_message_weekday")
