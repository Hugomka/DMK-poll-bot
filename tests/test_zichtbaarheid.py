...
from apps.utils.poll_settings import set_visibility, should_hide_counts
...
class TestZichtbaarheid(BaseTestCase):
    ...
    async def test_default_zichtbaarheid_altijd(self):
        nu = datetime(
            2025, 8, 16, 12, 0, tzinfo=ZoneInfo("Europe/Amsterdam")
        )  # zaterdag
    ...
    async def test_deadline_voor_18u(self):
        ...
        nu = datetime(
            2025, 8, 15, 17, 0, tzinfo=ZoneInfo("Europe/Amsterdam")
        )  # vrijdag 17:00
    ...
    async def test_deadline_na_18u(self):
        set_visibility(self.channel_id, self.dag, modus="deadline", tijd="18:00")
        nu = datetime(
            2025, 8, 15, 19, 0, tzinfo=ZoneInfo("Europe/Amsterdam")
        )  # vrijdag 19:00
        self.assertFalse(should_hide_counts(self.channel_id, self.dag, nu))
    ...
    async def test_onbekende_dag_geeft_false(self):
        ...
        self.assertFalse(
            should_hide_counts(self.channel_id, "maansdag", nu)
        )  # dag bestaat niet
